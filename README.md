...
| 管理员功能增加 | 2026年4月27日 | 1.增加一个管理员页面，对游戏表、用户表、二手回收遍进行管理<br />2.优化签到功能 |
| 登录系统与页面优化 | 2026年4月28日 | 1.增加退出登录功能<br />2.优化各项页面，删除冗余代码.<br />3.添加模糊查询 |
...
2.在 mysql 中创建数据库 `ps5game`，运行项目，将自动注入数据。如需关闭此功能，请将 `application.yml` 中的 `spring.datasource.initialization-mode=always` 代码删除。（自动注入功能尚未测试，请手动创建数据库，sql文件已经在data.sql里面整好了）
